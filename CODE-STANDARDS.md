...
- Препроцессинг и разрешение имён — требования к сборке. Каждая активная ссылка должна иметь объявление, доступное в данной конфигурации сборки. Отладочные объявления, их использование и очистка должны включаться и отключаться вместе.
...
- Методы классов, не выполняющие никаких действий и возвращающие константное значение, должны быть определены через макрос `getterconst_func(name,value)`. Неизменяемые признаки класса задавайте такими геттерами, а не полями `var` каждого экземпляра; обращайтесь к ним через `callSelf` или `callFunc`.
