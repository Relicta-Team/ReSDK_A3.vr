...
`CraftStation` хранит общий объём сбора и состояние работы; `ArtificerWorkbench`
реализует станцию `locks`. Старые `Workbench` и `LockWorkbench` наследуют её, поэтому
существующие размещения работают без изменения карт. Ингредиенты берутся со
столешницы, станция допускает одного работающего и повторно проверяет условия перед
расходом. Обычные рецепты не показываются в меню этой станции.
...
определены в `Crafts/Locks.yml`; там же находятся отдельные рецепты ремонта простого
и прочного сломанного замка без создания нового ключа.
