...
- **failed** - обработка провала крафта
...
required:
  ...
  force_visible: true # По умолчанию выключено

  #секция для ограничения по навыкам
  skills: 
    cavelore: 2 # требуется знание пещер не меньше 2х
  
  #секция компонентов (ингредиентов крафта)
  components:
    - class: Item # класснейм предмета\структуры.
      
      # Количество необходимых предметов. Не может быть меньше 1...
      count: 1 # по умолчанию 1.
      
      # Сколько должно быть хп у предмета чтобы он мог считаться ингредиентом.
      ...
      # внутренний тег ингредиента. используется в результате
      # ! Зарезервирован на будущее
      meta_tag: ITEM_1
```

### failed
В этом разделе настраивается провальная ситуация крафта. По умолчанию выключена. Провальная ситуация в основном подразумевает поломку\уничтожение ингредиентов при неудачном создании и\или нехватке навыков.
```yml
failed:
  enable: true # порча предметов при недостаточном навыке.
  item: Debris
...
result: # может содержать несколько выходных объектов
  ...
  # в примере используется выдуманный модификатор
  modifiers:
...
options:
  ...
  collect_distance: 4 # По умолчанию 1
  
  # Длительность прогресс-бара при создании.
  # Поддерживает математические выражения со статистикой персонажа rta (RealTimeAction), например при rta 3.5: 
  #     rta * 2 + 1
  # что означает (3.5 * 2 + 1) -> 7 + 1 -> 8, что примерно в 2 раза медленнее чем обычное стандартное действие персонажа
  craft_duration: rta * 2 # По умолчанию rta
```
