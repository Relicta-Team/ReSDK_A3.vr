...
| count | Количество создаваемых объектов. Свойство опционально. Может быть числом и диапазоном. Для диапазона укажите список из двух элементов: нижний предел и верхний. (`count: [3,7]` означает от 3 до 7 предметов) | 1 |
...
Например `cooking: 4`, что означает требует навыка готовки не меньше 4.
...
| can_fail | Может ли крафт быть провален | false |
| on_fail_item | Тип предмета, создаваемый при неудачной попытке крафта | - |
| on_fail_count | Количество предметов, создаваемых при неудачном крафте. | - |
