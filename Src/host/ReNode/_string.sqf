...
"
    ...
    desc:Форматирует строку. Указанные элементы внутри строки можно вывести с помощью %1. Если параметр ""
    icon:data\\icons\\icon_BluePrintEditor_Function_16px
    color:PureFunction
    exec:pure
    ...
    in:bool:Массив в элементы:При указании этого параметра как ИСТИНА переданный массив в порт Элемент будет разбит на элементы, каждый из которых можно подставить в строку: %1 - для первого элемента, %2 для второго и так далее.
        opt:def=false
