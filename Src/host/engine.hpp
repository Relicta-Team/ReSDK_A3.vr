...
/*
	...
	code - код помещаемый при компиляции. дефолт func(@thisMethod) {objParams();}
		доступные замены в коде
		@thisMethod - имя этого метода
		@thisParams - структура параметров
	...
	in - номер параметра для input-ов
		...
		opt (указывается на следующей строке):
			...
			pathes - доступные пути с разделителем |
...
/*
	...
	custom - пользовательский словарь опции
