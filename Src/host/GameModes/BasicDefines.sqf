...
class(GMBase) extends(IGameEvent) attribute(Story)
	...
	"
		...
		"
		code: func(@thisMethod) { @thisParams; @gvar = _countInGame; @gvar = _countProbFullAntags;  }
	...
	"
		...
		code: func(@thisMethod) { @thisParams; @gvar = _countInGame; @gvar = _countProbHiddenAntags; @out.1};
	...
	"
		...
		type:event
		code:func(@thisMethod) {@thisParams; @out.1; getSelf(finishResult)}
	" node_met
	...
	"
		...
		desc:Устанавливает результат конца раунда. Любое число, отличное от 0 является концом раунда. Соответственно 0 означает, что раунд должен продолжаться\n"+
		"Если результат уже установлен, то эта функция перезапишет его.
		out:int:Результат:Результат конца раунда
	" node_met
	...
	"
		...
		desc:Данное событие срабатывает когда раунд завершается.
		type:event
	" node_met
