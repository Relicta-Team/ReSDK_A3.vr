...
class(GMKrasaava) extends(GMBase) //GMBase - базовый режим, от которого унаследованы все возможные режимы.
	
    // -----------------------------------------------------------------//
    // ------------------ Базовые настройки режима ---------------------//
    // -----------------------------------------------------------------//

    // Текстовые данные режима GMKrasaava
	var(name,"Имя режима");
	var(desc,"Описание");
	var(descExtended,"Описание в конце игры.");

	// Длительность игры в режиме GMKrasaava.
	var(duration,t_asMin(30)); // 30 минут
    
    // Имя карты, которая будет создана в режиме GMKrasaava
	getterconst_func(getMapName,"reqwpoiq");
	...
	// Список ролей (имён классов) для режима GMKrasaava, доступных до старта раунда.
	func(getLobbyRoles)
	{
		[
            ...
            /*
                Пример:
                    "GMKrasaava_BasicRole1",
                    "GMKrasaava_BasicRole2",
                    "GMKrasaava_BasicRole3"
            */
		]
	};
	...
	func(conditionToStart)
	{
        ...
        /*
            Как правило здесь описываются условия типа: 
                если на "GMKrasaava_BasicRole1" встало 2 игрока и на "GMKrasaava_BasicRole2" встал 1 игрок,
            ...
            callSelfParams(getCandidatesCount,"GMKrasaava_BasicRole1") >= 2 
            && callSelfParams(getCandidatesCount,"GMKrasaava_BasicRole2") >= 1
	...
	func(onFailReasonToStart)
	{
		...
		"Не установлено свойство onFailReasonToStart() для режима GMKrasaava"
	};
	...
	// Вызывается когда режим GMKrasaava выбран (до старта раунда с загруженной картой)
	func(preSetup)
	{
		objParams();
	...
	// Событие при завершении раунда в режиме GMKrasaava
	func(onFinish)
	{
		objParams();
