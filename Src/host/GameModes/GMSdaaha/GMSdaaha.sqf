...
class(GMSdaaha) extends(GMBase) //GMBase - базовый режим, от которого унаследованы все возможные режимы.
	
    // -----------------------------------------------------------------//
    // ------------------ Базовые настройки режима ---------------------//
    // -----------------------------------------------------------------//

    // Текстовые данные режима GMSdaaha
	var(name,"Имя режима");
	var(desc,"Описание");
	var(descExtended,"Описание в конце игры.");

	// Длительность игры в режиме GMSdaaha.
	var(duration,t_asMin(30)); // 30 минут
    
    // Имя карты, которая будет создана в режиме GMSdaaha
	getterconst_func(getMapName,"SandboxMap");
	...
	// Список ролей (имён классов) для режима GMSdaaha, доступных до старта раунда.
	func(getLobbyRoles)
	{
		[
	...
	func(conditionToStart)
	{
        ...
        /*
            Как правило здесь описываются условия типа: 
                если на "GMSdaaha_BasicRole1" встало 2 игрока и на "GMSdaaha_BasicRole2" встал 1 игрок,
            ...
            callSelfParams(getCandidatesCount,"GMSdaaha_BasicRole1") >= 2 
            && callSelfParams(getCandidatesCount,"GMSdaaha_BasicRole2") >= 1
	...
	func(onFailReasonToStart)
	{
		...
		"Не установлено свойство onFailReasonToStart() для режима GMSdaaha"
	};
	...
	// Вызывается когда режим GMSdaaha выбран (до старта раунда с загруженной картой)
	func(preSetup)
	{
		objParams();
	...
	// Событие при завершении раунда в режиме GMSdaaha
	func(onFinish)
	{
		objParams();
