...
class(GMRatsAndCrown) extends(GMBase) //GMBase - базовый режим, от которого унаследованы все возможные режимы.
	
    // -----------------------------------------------------------------//
    // ------------------ Базовые настройки режима ---------------------//
    // -----------------------------------------------------------------//

    // Текстовые данные режима GMRatsAndCrown
	var(name,"Имя режима");
	var(desc,"Описание");
	var(descExtended,"Описание в конце игры.");

	// Длительность игры в режиме GMRatsAndCrown.
	var(duration,t_asMin(30)); // 30 минут
    
    // Имя карты, которая будет создана в режиме GMRatsAndCrown
	getterconst_func(getMapName,"ThroneRoomV1");
	...
	// Список ролей (имён классов) для режима GMRatsAndCrown, доступных до старта раунда.
	func(getLobbyRoles)
	{
		[
            ...
            /*
                Пример:
                    "GMRatsAndCrown_BasicRole1",
                    "GMRatsAndCrown_BasicRole2",
                    "GMRatsAndCrown_BasicRole3"
            */
		]
	};
    ...
    // ---- Доп. настройки для GMRatsAndCrown ----
	...
	func(conditionToStart)
	{
        ...
        /*
            Как правило здесь описываются условия типа: 
                если на "GMRatsAndCrown_BasicRole1" встало 2 игрока и на "GMRatsAndCrown_BasicRole2" встал 1 игрок,
            ...
            callSelfParams(getCandidatesCount,"GMRatsAndCrown_BasicRole1") >= 2 
            && callSelfParams(getCandidatesCount,"GMRatsAndCrown_BasicRole2") >= 1
	...
	func(onFailReasonToStart)
	{
		...
		"Не установлено свойство onFailReasonToStart() для режима GMRatsAndCrown"
	};
	...
	// Вызывается когда режим GMRatsAndCrown выбран (до старта раунда с загруженной картой)
	func(preSetup)
	{
		objParams();
	...
	// Событие при завершении раунда в режиме GMRatsAndCrown
	func(onFinish)
	{
		objParams();
