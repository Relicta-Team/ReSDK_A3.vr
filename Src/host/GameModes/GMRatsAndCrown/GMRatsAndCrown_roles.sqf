...
// Базовая роль для режима GMRatsAndCrown. Рекомендуется именовать все дочерние роли для этого режима с постфиксом имени режима и префиксом R (Role)
// Пример: RCaveExplorer_GMRatsAndCrown
class(GMRatsAndCrown_BasicRole) extends(BasicRole) // BasicRole - базовая роль, от которой унаследованы все возможные роли.
	...
	// Установите true требуется для тех ролей, без которых режим GMRatsAndCrown не сможет полноценно функционировать.
...
/*
	Определяем роли, унаследованные от GMRatsAndCrown_BasicRole

class(RDebugRole_GMRatsAndCrown) extends(GMRatsAndCrown_BasicRole)
	//change if need
	var(name,"Debug role");
