...
class(TBase) extends(IGameEvent)
	...
	"
		...
		desc:Возвращает @[bool ИСТИНУ], если задача завершила выполнение. Провал задачи также считается завершением.
		prop:get
	...
	"
		...
		desc:Значение, отвечающее за то когда будет проверен результат выполнения задачи. Если @[bool ИСТИНА] - задача проверяется с момента получения до выполнения. Если @[bool ЛОЖЬ] - полученная задача будет проверена только в конце раунда.
	" node_var
