...
// Базовая роль для режима GMLowTown_Cult. Рекомендуется именовать все дочерние роли для этого режима с постфиксом имени режима и префиксом R (Role)
// Пример: RCaveExplorer_GMLowTown_Cult
class(GMLowTown_Cult_BasicRole) extends(BasicRole) // BasicRole - базовая роль, от которой унаследованы все возможные роли.
	...
	// Установите true требуется для тех ролей, без которых режим GMLowTown_Cult не сможет полноценно функционировать.
...
/*
	Определяем роли, унаследованные от GMLowTown_Cult_BasicRole

class(RDebugRole_GMLowTown_Cult) extends(GMLowTown_Cult_BasicRole)
	//change if need
	var(name,"Debug role");
