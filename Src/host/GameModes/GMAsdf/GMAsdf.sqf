...
class(GMAsdf) extends(GMBase) //GMBase - базовый режим, от которого унаследованы все возможные режимы.
	
    // -----------------------------------------------------------------//
    // ------------------ Базовые настройки режима ---------------------//
    // -----------------------------------------------------------------//

    // Текстовые данные режима GMAsdf
	var(name,"Имя режима");
	var(desc,"Описание");
	var(descExtended,"Описание в конце игры.");

	// Длительность игры в режиме GMAsdf.
	var(duration,t_asMin(30)); // 30 минут
    
    // Имя карты, которая будет создана в режиме GMAsdf
	...
	// Список ролей (имён классов) для режима GMAsdf, доступных до старта раунда.
	func(getLobbyRoles)
	{
		[
	...
	func(conditionToStart)
	{
        ...
        /*
            Как правило здесь описываются условия типа: 
                если на "GMAsdf_BasicRole1" встало 2 игрока и на "GMAsdf_BasicRole2" встал 1 игрок,
            ...
            callSelfParams(getCandidatesCount,"GMAsdf_BasicRole1") >= 2 
            && callSelfParams(getCandidatesCount,"GMAsdf_BasicRole2") >= 1
	...
	func(onFailReasonToStart)
	{
		...
		"Не установлено свойство onFailReasonToStart() для режима GMAsdf"
	};
	...
	// Вызывается когда режим GMAsdf выбран (до старта раунда с загруженной картой)
	func(preSetup)
	{
		objParams();
	...
	// Событие при завершении раунда в режиме GMAsdf
	func(onFinish)
	{
		objParams();
