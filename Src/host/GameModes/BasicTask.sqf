...
class(TaskBase) extends(IGameEvent)
	...
	"
		...
		desc:Задача, в котоой это значение @[bool ИСТИНА] - будет автоматически провалена с результатом -5, если погибли все её владельцы. Обратите внимание, что со значением @[bool ИСТИНА] узла 'Единоразовая проверка условий' для провала задачи должен погибнуть хотя бы один её владелец.
		prop:all
