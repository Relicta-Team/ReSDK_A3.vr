...
class(GMPrisonLife) extends(GMBase) //GMBase - базовый режим, от которого унаследованы все возможные режимы.
	
    // -----------------------------------------------------------------//
    // ------------------ Базовые настройки режима ---------------------//
    // -----------------------------------------------------------------//

    // Текстовые данные режима GMPrisonLife
	var(name,"Имя режима");
	var(desc,"Описание");
	var(descExtended,"Описание в конце игры.");

	// Длительность игры в режиме GMPrisonLife.
	var(duration,t_asMin(30)); // 30 минут
    
    // Имя карты, которая будет создана в режиме GMPrisonLife
	getterconst_func(getMapName,"NRPrison");
	...
	// Список ролей (имён классов) для режима GMPrisonLife, доступных до старта раунда.
	func(getLobbyRoles)
	{
		[
            ...
            /*
                Пример:
                    "GMPrisonLife_BasicRole1",
                    "GMPrisonLife_BasicRole2",
                    "GMPrisonLife_BasicRole3"
            */
		]
	};
    ...
    // ---- Доп. настройки для GMPrisonLife ----
	...
	func(conditionToStart)
	{
        ...
        /*
            Как правило здесь описываются условия типа: 
                если на "GMPrisonLife_BasicRole1" встало 2 игрока и на "GMPrisonLife_BasicRole2" встал 1 игрок,
            ...
            callSelfParams(getCandidatesCount,"GMPrisonLife_BasicRole1") >= 2 
            && callSelfParams(getCandidatesCount,"GMPrisonLife_BasicRole2") >= 1
	...
	func(onFailReasonToStart)
	{
		...
		"Не установлено свойство onFailReasonToStart() для режима GMPrisonLife"
	};
	...
	// Вызывается когда режим GMPrisonLife выбран (до старта раунда с загруженной картой)
	func(preSetup)
	{
		objParams();
	...
	// Событие при завершении раунда в режиме GMPrisonLife
	func(onFinish)
	{
		objParams();
