...
// Базовая роль для режима GMNAPrisonWorkShift. Рекомендуется именовать все дочерние роли для этого режима с постфиксом имени режима и префиксом R (Role)
// Пример: RCaveExplorer_GMNAPrisonWorkShift
class(GMNAPrisonWorkShift_BasicRole) extends(BasicRole) // BasicRole - базовая роль, от которой унаследованы все возможные роли.
	...
	// Установите true требуется для тех ролей, без которых режим GMNAPrisonWorkShift не сможет полноценно функционировать.
...
/*
	Определяем роли, унаследованные от GMNAPrisonWorkShift_BasicRole

class(RDebugRole_GMNAPrisonWorkShift) extends(GMNAPrisonWorkShift_BasicRole)
	//change if need
	var(name,"Debug role");
