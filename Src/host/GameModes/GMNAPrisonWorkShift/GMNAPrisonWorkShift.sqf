...
class(GMNAPrisonWorkShift) extends(GMBase) //GMBase - базовый режим, от которого унаследованы все возможные режимы.
	
    // -----------------------------------------------------------------//
    // ------------------ Базовые настройки режима ---------------------//
    // -----------------------------------------------------------------//

    // Текстовые данные режима GMNAPrisonWorkShift
	var(name,"Имя режима");
	var(desc,"Описание");
	var(descExtended,"Описание в конце игры.");

	// Длительность игры в режиме GMNAPrisonWorkShift.
	var(duration,t_asMin(30)); // 30 минут
    
    // Имя карты, которая будет создана в режиме GMNAPrisonWorkShift
	getterconst_func(getMapName,"NAPrisonBoxV1");
	...
	// Список ролей (имён классов) для режима GMNAPrisonWorkShift, доступных до старта раунда.
	func(getLobbyRoles)
	{
		[
            ...
            /*
                Пример:
                    "GMNAPrisonWorkShift_BasicRole1",
                    "GMNAPrisonWorkShift_BasicRole2",
                    "GMNAPrisonWorkShift_BasicRole3"
            */
		]
	};
    ...
    // ---- Доп. настройки для GMNAPrisonWorkShift ----
	...
	func(conditionToStart)
	{
        ...
        /*
            Как правило здесь описываются условия типа: 
                если на "GMNAPrisonWorkShift_BasicRole1" встало 2 игрока и на "GMNAPrisonWorkShift_BasicRole2" встал 1 игрок,
            ...
            callSelfParams(getCandidatesCount,"GMNAPrisonWorkShift_BasicRole1") >= 2 
            && callSelfParams(getCandidatesCount,"GMNAPrisonWorkShift_BasicRole2") >= 1
	...
	func(onFailReasonToStart)
	{
		...
		"Не установлено свойство onFailReasonToStart() для режима GMNAPrisonWorkShift"
	};
	...
	// Вызывается когда режим GMNAPrisonWorkShift выбран (до старта раунда с загруженной картой)
	func(preSetup)
	{
		objParams();
	...
	// Событие при завершении раунда в режиме GMNAPrisonWorkShift
	func(onFinish)
	{
		objParams();
