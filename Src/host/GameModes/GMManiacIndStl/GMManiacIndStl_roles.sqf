...
// Базовая роль для режима GMManiacIndStl. Рекомендуется именовать все дочерние роли для этого режима с постфиксом имени режима и префиксом R (Role)
// Пример: RCaveExplorer_GMManiacIndStl
class(GMManiacIndStl_BasicRole) extends(BasicRole) // BasicRole - базовая роль, от которой унаследованы все возможные роли.
	...
	// Установите true требуется для тех ролей, без которых режим GMManiacIndStl не сможет полноценно функционировать.
...
/*
	Определяем роли, унаследованные от GMManiacIndStl_BasicRole

class(RDebugRole_GMManiacIndStl) extends(GMManiacIndStl_BasicRole)
	//change if need
	var(name,"Debug role");
