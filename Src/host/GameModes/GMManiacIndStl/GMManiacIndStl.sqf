...
class(GMManiacIndStl) extends(GMBase) //GMBase - базовый режим, от которого унаследованы все возможные режимы.
	
    // -----------------------------------------------------------------//
    // ------------------ Базовые настройки режима ---------------------//
    // -----------------------------------------------------------------//

    // Текстовые данные режима GMManiacIndStl
	var(name,"Имя режима");
	var(desc,"Описание");
	var(descExtended,"Описание в конце игры.");

	// Длительность игры в режиме GMManiacIndStl.
	var(duration,t_asMin(30)); // 30 минут
    
    // Имя карты, которая будет создана в режиме GMManiacIndStl
	getterconst_func(getMapName,"IndustrialStl_1");
	...
	// Список ролей (имён классов) для режима GMManiacIndStl, доступных до старта раунда.
	func(getLobbyRoles)
	{
		[
            ...
            /*
                Пример:
                    "GMManiacIndStl_BasicRole1",
                    "GMManiacIndStl_BasicRole2",
                    "GMManiacIndStl_BasicRole3"
            */
		]
	};
    ...
    // ---- Доп. настройки для GMManiacIndStl ----
	...
	func(conditionToStart)
	{
        ...
        /*
            Как правило здесь описываются условия типа: 
                если на "GMManiacIndStl_BasicRole1" встало 2 игрока и на "GMManiacIndStl_BasicRole2" встал 1 игрок,
            ...
            callSelfParams(getCandidatesCount,"GMManiacIndStl_BasicRole1") >= 2 
            && callSelfParams(getCandidatesCount,"GMManiacIndStl_BasicRole2") >= 1
	...
	func(onFailReasonToStart)
	{
		...
		"Не установлено свойство onFailReasonToStart() для режима GMManiacIndStl"
	};
	...
	// Вызывается когда режим GMManiacIndStl выбран (до старта раунда с загруженной картой)
	func(preSetup)
	{
		objParams();
	...
	// Событие при завершении раунда в режиме GMManiacIndStl
	func(onFinish)
	{
		objParams();
