...
class(RSmotriyashchy) extends(BasicRole)
	...
	{
		objParams_1(_mob);
		private _msg = "Будучи одним из первых жителей Забродка и самым смекалистым — ты сумел сделать себе имя." + sbr +
			"Когда-то давно ты хотел попасть в город, а теперь зарабатываешь почти так же много, как и Торгаш: ведь под твоим владением бар ""Прохлада"", единственное место Забродка, где готовят еду и подают пещерный алкоголь уставшим путникам." + sbr +
			"Но с властью приходит ответственность: тебе иногда приходится подгонять работу всяческим хвостам, чтобы те не перерезали друг-друга, и за тобой последнее слово в конфликтах и спорах Забродка.";
		callFuncParams(_mob,addFirstJoinMessage,_msg);
	};
endclass
...
class(RUshibala) extends(BasicRole)
	...
	{
		objParams_1(_mob);
		private _msg = "Ты сам себе правая рука. Сильный, выносливый и достаточно смышлёный, чтобы понять, что пока существует Забродок — существует необходимость поддержания в нём порядка, за который Смотрящий ещё готов и заплатить." + sbr +
			"Работайте вместе, чтобы привести Забродок и друг-друга к обогащению.";
		callFuncParams(_mob,addFirstJoinMessage,_msg);
	};
endclass
...
class(RNomadZabrodok) extends(BasicRole)
	...
	{
		objParams_1(_mob);
		private _msg = "Будучи кочевником, ты пришёл в Грязноямск, но по какой-то причине тебя не пустили." + sbr +
			"Неподалёку от Грязноямска есть Забродок — перевалочный пункт для путников, предоставляющий ночлег, припасы и очаг тем, которые разделили с тобой участь отверженного." + sbr +
			"И в эту смену ты снова попытаешь счастье: либо на гермовратах, либо в Забродке.";
		callFuncParams(_mob,addFirstJoinMessage,_msg);
	};
