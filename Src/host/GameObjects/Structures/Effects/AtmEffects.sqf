...
class(EffectAsStruct) extends(ILightibleStruct)
	...
	func(getEffectEnum)
	{
		...
		private _efenm = [SLIGHT_EFF_DUST_PARTICLES,SLIGHT_EFF_DUST_CLOUDS,SLIGHT_EFF_DUST_GREEN,SLIGHT_EFF_DUST_ROTTEN_HUMAN];
		...
		if (_idx == -1) exitWith {
			...
			SLIGHT_EFF_DUST_PARTICLES //some random enum effect
		};
