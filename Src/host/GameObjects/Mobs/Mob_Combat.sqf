...
region(shooting)
	...
	// Обработчик события обновления последнего атакующего
	// Вызывается при каждом обновлении информации о том, кто атаковал моба последним
	// Параметры: _attacker - атакующий (моб или объект), _weapon - оружие/метод атаки, _optItem - опциональный предмет
	func(onLastAttackerUpdated)
	{
		objParams_3(_attacker,_weapon,_optItem);
		// Базовая реализация - пустая
		// Может быть переопределена в наследниках для реакции на атаку
	};
