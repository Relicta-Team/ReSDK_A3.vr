...
class(@GAMEMODE_NAME@_BasicRole) extends(BasicRole) // BasicRole - базовая роль, от которой унаследованы все возможные роли.
    ...
    */
	...
	// Функция, в которой можно вызать экипировку персонажу
	func(getEquipment)
	{
		objParams_1(_mob);
		/*
			Чтобы создать пердмет в слоте персонажа используйте createItemInInventory
			Чтобы создать предмет внутри контейнера (одежда, рюкзак) используй createItemInContainer
		*/
	};
	...
	*/
	...
	// Функция, вызывающаяся когда умирает персонаж владеющий этой ролью.
	/*
		Если игрок зашёл за персонажа за роль A и потом получил роль B,
	...
	*/
	...
	!Внимание! 
		При модификации очков клиента второй параметр _onlyOnGame должен быть ТОЛЬКО false,
		так как данные методы вызываются когда стейт игры уже изменился на GAME_STATE_END
		
		См.: ServerCleint::addPoints()
	*/
	func(onEndgame)
	{
		objParams_2(_mob,_usr);
