...
if (process.env.npm_config_project) {
	proj = process.env.npm_config_project;
}
...
function run(proj, mode) {
	...
	if (!fs.existsSync(projectDir)) {
		console.error(colorize(`Project ${proj} not found: ${projectDir}`, colors.red));
		process.exit(1);
	}
  ...
  if (mode === 'ts') {
	let entryFile = path.join(projectDir, 'index.ts');
...
if (!proj || !mode) {
  ...
  rl.question('Enter project name (proj): ', (projAnswer) => {
    ...
    rl.question('Enter mode (ts/serve): ', (modeAnswer) => {
      ...
      rl.close();
      if (!proj || !mode) {
        console.error('Usage: npm run runproj -- --project=projName --mode=ts|serve');
        process.exit(1);
      }
      run(proj, mode);
    });
