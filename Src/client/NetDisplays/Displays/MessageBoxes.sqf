...
ND_INIT(MessageBox)
	...
	ctxParams params [["_text",""],["_ok","ОК"],["_can","Закрыть"]];
	...
	_textWid = [thisDisplay,[50.5,81,44,18],_ctg,true] call nd_addClosingButton;
...
ND_INIT(Listbox)	
	...
	private _txtval = ctxParams deleteAt 0;
...
ND_INIT(Text)
	...
	ctxParams params [["_text",""],["_close","Закрыть"]];
	...
	_textWid = [thisDisplay,[5,91,90,8],_ctg,true] call nd_addClosingButton;
...
ND_INIT(Alert)
	...
	ctxParams params [["_text",""],["_ok","Да"],["_can","Нет"]];
...
ND_INIT(Input)
	...
	ctxParams params [["_t",""],["_def",""],["_ok","ОК"],["_can","Закрыть"],["_ischatmode",false]];
	...
	_textWid = [thisDisplay,[52,86,46,13],_ctg,true] call nd_addClosingButton;
	_textWid ctrlSetText _can;
	if (_ischatmode) then {
		[_ctg,
			[
		...
		ctrlSetFocus _input;
		if isNull(thisDisplay getvariable "___event_onpress") then {
			thisDisplay setvariable ["___buttonSend",_buttonSend];
			thisDisplay setvariable ["___event_onpress",thisDisplay displayAddEventHandler ["KeyUp",{
				params ["_d","_key"];
...
ND_INIT(TestingReputation)
	...
	private _tDat = ctxParams deleteAt 0;
	forceUnicode 1;
	...
	private _data = "";
	private _item = "";
	private _items = array_shuffle(ctxParams deleteAt 0);
