...
namespace intercept {
    namespace sqf {
        ...
        struct rv_target_knowledge {
            ...
            bool known_by_unit;
            /**
                   * @brief Constructs an rv_target_knowledge by deserializing its fields from a game_value.
                   *
                   * Extracts values from gv_ in the following order and assigns them to the corresponding members:
                   * - gv_[0] -> known_by_group
                   * - gv_[1] -> known_by_unit
                   * - gv_[2] -> last_seen_by_unit
                   * - gv_[3] -> last_endangered_by_unit
                   * - gv_[4] -> target_side
                   * - gv_[5] -> position_error
                   * - gv_[6] -> target_position
                   *
                   * @param gv_ Source game_value containing serialized rv_target_knowledge fields in the order described above.
                   */
        ...
        struct rv_vision_mode {
            ...
            int FLIR_index;
            /**
                 * @brief Construct an rv_vision_mode from a game_value array.
                 *
                 * Initializes vision_mode and FLIR_index from the provided game_value where
                 * index 0 is the vision mode and index 1 is the FLIR index.
                 *
                 * @param gv_ game_value array containing [vision_mode, FLIR_index].
                 */
        ...
        struct rv_move_info {
            ...
            float gesture_factor;    /**
                                                            * @brief Constructs rv_move_info by extracting movement and gesture data from a game_value.
                                                            *
                                                            * Initializes members from elements of gv_ in the following order:
                                                            * 0: move_progress, 1: move_time, 2: move_duration, 3: move_factor, 4: rtm_step,
                                                            * 5: gesture_progress, 6: gesture_time, 7: gesture_duration, 8: gesture_factor.
                                                            *
                                                            * @param gv_ Game value array containing the nine elements in the order above.
                                                            */
        ...
        */
        ...
        struct rv_freefall_info {
            ...
            /**
                 * @brief Represents an object's freefall state and initializes it from a game_value.
                 *
                 * Constructs rv_freefall_info by extracting values from a game_value array in the order:
                 * [is_falling, is_in_halo_pose, min_height].
                 *
                 * @param gv_ game_value array where:
                 *            - gv_[0]: boolean indicating whether the unit is currently falling,
                 *            - gv_[1]: boolean indicating whether the unit is in halo pose,
                 *            - gv_[2]: float specifying the minimum height for freefall.
                 */
        ...
        struct rv_weapons_info {
            ...
            bool is_primary;           /**
                   * @brief Construct an rv_weapons_info by deserializing a game_value array.
                   *
                   * Initializes struct members from the provided `game_value` where elements are
                   * expected in the following order:
                   * 0: weapon_index, 1: is_selected, 2: weapon_name, 3: muzzle_name,
                   * 4: fire_mode, 5: magazine_name, 6: ammo_count, 7: is_primary.
                   *
                   * @param gv_ Array-like `game_value` containing the fields in the order described above.
                   */
        ...
        struct rv_player_target_lock {
            ...
            float lock_value;  /**
                                                                     * @brief Constructs an rv_player_target_lock from a serialized game value.
                                                                     *
                                                                     * Initializes the struct's fields by reading elements from the provided `game_value` in the
                                                                     * following order: target <- gv[0], lock_value <- gv[1], config <- gv[2].
                                                                     *
                                                                     * @param gv_ A `game_value` containing [target, lock_value, config] in that order.
                                                                     *              `lock_value` is in the range 0..1 where 1 indicates fully locked.
                                                                     */
