...
namespace intercept {
    namespace sqf {
        ...
        struct rv_audio_options {
            ...
            /**
             * @brief Construct audio option volumes from a game_value container.
             *
             * Initializes the effects, music, radio, and von fields from the provided game_value,
             * which is expected to be an indexable container where:
             * - index 0 -> effects
             * - index 1 -> music
             * - index 2 -> radio
             * - index 3 -> von
             *
             * @param data_ Container supplying the four audio option volume values in the order above.
             */
        ...
        struct rv_sound_controller {
            ...
            float value;
            /**
 * @brief Constructs an rv_sound_controller from a game_value.
 *
 * Initializes the struct by extracting the controller name from gv_[0] and
 * the controller numeric value from gv_[1].
 *
 * @param gv_ A game_value array where index 0 is the controller name and index 1 is the controller value.
 */
explicit rv_sound_controller(const game_value &gv_) : name(gv_[0]), value(gv_[1]) {}
        ...
        struct rv_sound_params {
            ...
            float volume;     /**
                   * @brief Construct rv_sound_params from a game_value containing sound parameter fields.
                   *
                   * Initializes the struct's members using elements of `gv_` where index 0 is the sound path,
                   * 1 is the current playback position, 2 is the total length, 3 is the playback time, and 4 is the volume.
                   *
                   * @param gv_ game_value expected to be an array-like container with at least five elements in the order:
                   *            [path, cur_pos, length, time, volume].
                   */
