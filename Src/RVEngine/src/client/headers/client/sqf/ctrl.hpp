...
namespace intercept {
    namespace sqf {
        ...
        struct rv_ctrl_angle {
            ...
            float y_center{};
            /**
                   * @brief Construct an rv_ctrl_angle from a game_value representation.
                   *
                   * Initializes the angle and center coordinates from the supplied game_value.
                   *
                   * @param gv_ A game_value vector of three numeric elements in order: angle, x_center, y_center.
                   */
        ...
        struct rv_ctrl_text_selection {
            ...
            float length{};
            /**
                   * @brief Construct an rv_ctrl_text_selection from a game_value array.
                   *
                   * Initializes selection start, length, and text from the provided game_value.
                   *
                   * @param gv_ game_value expected to be an indexable array where:
                   *            - gv_[0] is the selection start position,
                   *            - gv_[1] is the selection length,
                   *            - gv_[2] is the selected text.
                   */
        ...
        struct rv_lnb_array {
            ...
            /**
             * @brief Constructs a game_value representing this lnb array as three columns.
             *
             * Produces a nested game_value vector where the first element is the texts column,
             * the second is the numeric values column, and the third is the datas column.
             *
             * @return game_value A vector of three `game_value` vectors: texts, values, and datas (in that order).
             */
            operator game_value() {
            ...
            /**
             * @brief Produce a game_value containing the three columns of the lnb array.
             *
             * Converts the stored texts, numeric values, and datas into a nested game_value
             * vector where the first element is the texts array, the second is the values
             * array, and the third is the datas array.
             *
             * @return game_value A vector of three `game_value` arrays: texts, values, and datas (in that order).
             */
        ...
        struct rv_ctrl_position {
            float x, y, w, h;
            /**
             * @brief Constructs an rv_ctrl_position from a game_value containing four numeric components.
             *
             * Initializes the position's x, y, w, and h from gv_[0], gv_[1], gv_[2], and gv_[3], respectively.
             *
             * @param gv_ A game_value array where element 0 is x, 1 is y, 2 is w, and 3 is h. 
             */
        ...
        struct rv_resolution {
            ...
            /**
             * @brief Construct an rv_resolution from a game_value vector of values.
             *
             * Initializes resolution, viewport, aspect_ratio, ui_scale, fov, triple_head, and texture_quality
             * from consecutive elements of the provided game_value.
             *
             * @param gv_ game_value containing at least 10 elements with the following layout:
             *            - gv_[0], gv_[1]: resolution (width, height)
             *            - gv_[2], gv_[3]: viewport (width, height)
             *            - gv_[4]: aspect ratio
             *            - gv_[5]: UI scale
             *            - gv_[6], gv_[7]: field of view components (mapped to fov as {gv_[7], gv_[6]})
             *            - gv_[8]: triple-head flag
             *            - gv_[9]: texture quality
             */
        ...
        struct rv_subtitle_options {
            ...
            /**
             * @brief Construct from a game_value containing subtitle flags.
             *
             * Reads subtitle options from `data_`: `subtitles` is taken from `data_[0]` and
             * `radioSubtitles` is taken from `data_[1]`.
             *
             * @param data_ game_value with at least two elements where index 0 represents
             *               the subtitles flag and index 1 represents the radio subtitles flag.
             */
