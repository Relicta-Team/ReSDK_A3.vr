...
namespace intercept {
    namespace sqf {
        struct rv_action_key_details {
            ...
            /**
             * @brief Initializes action key details from a game_value array returned by the engine.
             *
             * Parses the provided `gv_` array to populate `main_device`, `combo_device`, `main_dik_code`,
             * `combo_dik_code`, `device_id`, and `is_double_tap`. If expected array elements are missing or
             * empty, the corresponding members remain at their default values.
             *
             * @param gv_ Engine-provided `game_value` holding action key details in the expected array layout.
             */
