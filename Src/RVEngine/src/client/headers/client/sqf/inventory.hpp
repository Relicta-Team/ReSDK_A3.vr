...
namespace intercept {
    namespace sqf {
        ...
        struct rv_magazine_ammo {
            ...
            /**
                                                            * @brief Constructs an rv_magazine_ammo from a serialized game_value.
                                                            *
                                                            * @param from_gv_ A two-element game_value where element 0 is the magazine name (string)
                                                            *                 and element 1 is the magazine count (numeric).
                                                            */
                                                           explicit rv_magazine_ammo(const game_value &from_gv_) : name(from_gv_[0]),
                                                           count(from_gv_[1]) {}

            /**
             * @brief Serialize the magazine entry as a two-element game_value.
             *
             * Produces a game_value array whose first element is the magazine name and
             * second element is the magazine count converted to a float.
             *
             * @return game_value An array in the form `[name, count]` where `count` is a float.
             */
        ...
        struct rv_magazine_ammo_full : rv_magazine_ammo {
            ...
            /**
                                                                 * @brief Constructs an rv_magazine_ammo_full from a serialized game_value.
                                                                 *
                                                                 * @param from_gv_ A game_value expected to be an array [name, count, loaded, type, location],
                                                                 *                 where `name` is the magazine name, `count` is the ammo count,
                                                                 *                 `loaded` is a boolean indicating whether the magazine is loaded,
                                                                 *                 `type` is an integer magazine type identifier, and `location` is the location string.
                                                                 */
            ...
            /**
                                                                        * @brief Constructs a full magazine-ammo record from a basic magazine-ammo.
                                                                        *
                                                                        * Copies the magazine name and count from `from_mag_`; sets `loaded` to `false`
                                                                        * and `type` to `-1`. Other members (e.g., `location`) are left at their defaults.
                                                                        *
                                                                        * @param from_mag_ Source `rv_magazine_ammo` whose name and count are copied.
                                                                        */
            ...
            /**
             * @brief Serialize the magazine entry into an engine-compatible array.
             *
             * The resulting array contains the magazine's name, count, loaded flag,
             * type identifier, and location in that order.
             *
             * @return game_value Array in the form: [name, count (float), loaded (bool), type (float), location].
             */
            operator game_value() const {
        ...
        struct rv_turret_magazine {
            ...
            /**
 * @brief Constructs an rv_turret_magazine from a serialized game_value.
 *
 * Initializes the turret magazine fields by deserializing a game_value expected to contain
 * five elements in order: name, turret path, count, id, and creator.
 *
 * @param gv_ Serialized representation containing [name, turret_path, count, id, creator].
 */
explicit rv_turret_magazine(game_value gv_) : name(gv_[0]),turret_path(gv_[1]),count(gv_[2]),id(gv_[3]),creator(gv_[4]){}
        ...
        struct rv_container {
            ...
            /**
                                                        * @brief Constructs an rv_container from a game_value representation.
                                                        *
                                                        * Expects `from_gv_` to be an array where element 0 is the container type (string)
                                                        * and element 1 is the container object. Initializes `type` and `container`
                                                        * from those elements.
                                                        *
                                                        * @param from_gv_ Source game_value with structure [type, container].
                                                        */
                                                       explicit rv_container(const game_value &from_gv_) : type(from_gv_[0]),
                                                       container(from_gv_[1]) {}
            /**
             * @brief Serialize this container to a game_value containing its type and container.
             *
             * @return game_value A two-element array: first element is the container type string, second element is the container serialized as a `game_value`.
             */
            operator game_value() const {
        ...
        struct rv_weapon_accessories {
            ...
            sqf_string bipod;
            /**
                   * @brief Initializes weapon accessory names from a serialized game_value.
                   *
                   * Constructs an rv_weapon_accessories by reading four string entries from the provided game_value:
                   * [silencer, flashlight_laserpointer, optics, bipod].
                   *
                   * @param gv_ game_value array expected to contain four elements in the order above; each element is converted to the corresponding member name.
                   */
        ...
        struct rv_weapon_state {
            ...
            /**
                                                                           * @brief Initialize a rv_weapon_state from a game_value.
                                                                           *
                                                                           * Constructs the struct by extracting, in order, weapon name, muzzle name,
                                                                           * firing mode, magazine name, ammo count, round reload phase, and magazine
                                                                           * reload phase from the provided game_value array.
                                                                           *
                                                                           * @param ret_game_value_ game_value array with seven elements in the order:
                                                                           *                         [weapon, muzzle, mode, magazine, ammo_count,
                                                                           *                          round_reload_phase, magazine_reload_phase].
                                                                           */
        ...
        struct rv_magazine {
            ...
            int max_ammo;       /**
                                                              * @brief Constructs an rv_magazine by deserializing a game_value array.
                                                              *
                                                              * Extracts elements in order: name, ammo, muzzle, max_ammo. Missing string elements become an empty string; missing numeric elements become -1.
                                                              *
                                                              * @param ret_game_value_ game_value array expected to contain [name, ammo, muzzle, max_ammo] (any trailing elements are ignored).
                                                              */
            ...
            /**
             * @brief Serialize the magazine to a two-element game_value containing its name and ammo.
             *
             * The conversion includes only the magazine `name` and `ammo` fields.
             *
             * @return game_value Array with two elements: the magazine name and the ammo count.
             */
        ...
        struct rv_weapon_items {
            ...
            /**
                                                                  * @brief Constructs an rv_weapon_items instance from a serialized game_value.
                                                                  *
                                                                  * @param ret_game_value_ A game_value array expected to contain, in order:
                                                                  *                        [0] weapon, [1] muzzle, [2] laser, [3] optics,
                                                                  *                        [4] primary_magazine, [5] secondary_magazine, [6] bipod.
                                                                  */
            ...
            /**
             * @brief Serialize the weapon items into a game_value array.
             *
             * Converts the weapon items structure into a game_value containing:
             * [weapon, muzzle, laser, optics, primary_magazine, secondary_magazine, bipod].
             *
             * @return game_value The serialized representation as an array in the order listed above.
             */
            explicit operator game_value() const {
        ...
        struct rv_throwable {
            ...
            float creator_id;
            /**
                                                            * @brief Constructs an rv_throwable from a game_value representation.
                                                            *
                                                            * @param gv_ A game_value with the form [magazine_name, muzzle_name, [world_id, creator_id]],
                                                            *            where `magazine_name` and `muzzle_name` are strings and the third element is a 2‑element
                                                            *            array containing the world ID and the creator ID respectively.
                                                            */
        ...
        struct rv_cargo {
            ...
            /**
             * @brief Convert this object into a game_value representation.
             *
             * Produces a two-element game_value array containing the cargo types and their corresponding amounts.
             *
             * @return game_value An array where element 0 is `types` (array of strings) and element 1 is `amounts` (array of numbers).
             */
            explicit operator game_value() const {
        ...
        struct rv_handgun_items {
            ...
            std::string optics;
            std::string bipod;  /**
                                                                            * @brief Constructs an rv_handgun_items by extracting accessory values from a game_value array.
                                                                            *
                                                                            * @param ret_game_value_ A game_value array expected to contain four elements in order:
                                                                            *                        index 0 -> silencer, index 1 -> laser, index 2 -> optics, index 3 -> bipod.
                                                                            */
            ...
            /**
             * @brief Serializes the weapon accessory selection into a game_value array.
             *
             * Produces a game_value containing the accessory entries in the order: silencer, laser, optics, bipod.
             *
             * @return game_value Array of four elements: `[silencer, laser, optics, bipod]`.
             */
            explicit operator game_value() const {
        ...
        struct rv_unit_description {
            ...
            /**
                   * @brief Constructs an rv_unit_description from a serialized game_value.
                   *
                   * Initializes the unit, uniform, vest, and backpack fields from the provided
                   * game_value which is expected to be an array with four elements in the
                   * following order: [unit, uniform, vest, backpack].
                   *
                   * @param r_game_val Serialized representation containing the four elements.
                   */
        ...
        struct rv_magazine_info {
            ...
            /**
                                                                   * @brief Constructs an rv_magazine_info from a game_value array.
                                                                   *
                                                                   * Parses the input array into magazine name, ammo and count using positional rules:
                                                                   * - element 0 -> magazine name (string) if present, otherwise empty string;
                                                                   * - if array size > 2: element 1 -> ammo (int), element 2 -> count (int);
                                                                   * - if array size == 2: ammo is set to -1, element 1 -> count (int);
                                                                   * - if array size < 2: ammo and count are set to -1.
                                                                   *
                                                                   * @param ret_game_value_ Source game_value array encoded as [magazine, ammo?, count?].
                                                                   */
            ...
            /**
             * @brief Initializes an rv_magazine_info with sentinel default values.
             *
             * Sets `ammo` and `count` to -1 to indicate an uninitialized/invalid state.
             */
            rv_magazine_info() {
                ammo = count = -1;
            }
            ...
            /**
             * @brief Serialize this magazine into a game_value array representation.
             *
             * Produces a compact game_value array reflecting the magazine name and available counts using sentinels.
             *
             * @return game_value
             * - If `ammo` != -1: an array `[magazine, ammo (float), count (float)]`.
             * - Else if `count` != -1: an array `[magazine, count (float)]`.
            ...
            /**
             * @brief Compare two rv_magazine_info objects for value equality.
             *
             * @param other The rv_magazine_info to compare with this instance.
             * @return `true` if `magazine`, `ammo`, and `count` are equal in both objects, `false` otherwise.
             */
            ...
            /**
             * @brief Resets the magazine entry to an empty, invalid state.
             *
             * Clears the magazine name and sets `ammo` and `count` to -1.
             */
            void clear() {
                magazine.clear();
                ammo = -1;
        ...
        struct rv_weapon_info {  //#TODO It would be better to store these as r_strings
            ...
            /**
 * @brief Initializes an rv_weapon_info with default member values.
 *
 * Constructs an rv_weapon_info where all fields are value-initialized (empty strings,
 * zeros, or their respective defaults).
 */
rv_weapon_info() = default;

            /**
             * @brief Constructs an rv_weapon_info from a serialized game_value array.
             *
             * Parses a game_value array produced by the engine and populates the struct fields.
             *
             * @param ret_game_value_ A game_value array with expected layout:
             *  - [0] weapon (string)
             *  - [1] silencer (string)
             *  - [2] laser (string)
             *  - [3] optics (string)
             *  - [4] primary_muzzle_magazine (game_value)
             *  - [5] secondary_muzzle_magazine (game_value)
             *  - [6] bipod (string)
             *
             * If the array is empty, the constructor leaves all fields at their defaults and sets `weapon` to an empty string.
             */
            ...
            /**
             * Serialize the weapon items into a game_value array.
             *
             * @return game_value An array containing [weapon, silencer, laser, optics, primary_muzzle_magazine, secondary_muzzle_magazine, bipod]; returns an empty array if `weapon` is empty.
             */
            explicit operator game_value() const {
                if (!weapon.empty()) {
            ...
            /**
             * @brief Compares two rv_weapon_info instances for value equality.
             *
             * Performs a field-wise comparison of weapon, silencer, laser, optics,
             * primary_muzzle_magazine, secondary_muzzle_magazine, and bipod.
             *
             * @param other The rv_weapon_info to compare against.
             * @return `true` if all compared fields are equal, `false` otherwise.
             */
            ...
            /**
             * @brief Reset all weapon item fields to their empty state.
             *
             * Clears the weapon, attachment (silencer, laser, optics, bipod) and magazine
             * fields so the struct contains no assigned weapon or accessories.
             */
            void clear() {
                weapon.clear();
                silencer.clear();
        ...
        struct rv_container_info {
            ...
            /**
 * @brief Constructs an empty container info.
 *
 * Initializes `container` to an empty string and `items` to an empty vector.
 */
rv_container_info() = default;

            /**
             * @brief Constructs an rv_container_info from a serialized game_value.
             *
             * Initializes the container name and item list from a game_value expected to be
             * an array where element 0 is the container name (string) and element 1 is an
             * array of item entries convertible to the container's item type.
             *
             * @param ret_game_value_ Serialized representation: [containerName, itemsArray].
             */
            ...
            /**
             * @brief Serialize the container info to a game_value array.
             *
             * Produces a serialized representation suitable for engine interop. If `container` is non-empty,
             * the result is an array where the first element is the container name and the second is an array
             * of the contained item values. If `container` is empty, an empty `game_value` is returned.
             *
             * @return game_value ` [container, items...]` when `container` is non-empty; empty `game_value` otherwise.
             */
            explicit operator game_value() const {
                if (!container.empty()) {
            ...
            /**
             * @brief Compare two rv_container_info instances for equality.
             *
             * @param other The rv_container_info to compare against.
             * @return `true` if both the `container` string and the `items` vector are equal, `false` otherwise.
             */
            bool operator==(const rv_container_info& other) const {
             ...
             /**
             * @brief Reset the container description to an empty state.
             *
             * Sets the container name to an empty string and removes all items.
             */
            void clear() {
                 container.clear();
                 items.clear();
        ...
        struct rv_unit_loadout {
            ...
            /**
 * @brief Constructs a default, empty unit loadout.
 *
 * Initializes all loadout fields to their default-constructed values (empty strings,
 * zero/false numeric flags, and empty containers).
 */
rv_unit_loadout() = default;

            /**
             * @brief Constructs an rv_unit_loadout from a serialized game_value representation.
             *
             * Initializes members from elements of ret_game_value_ in the following order:
             * primary, secondary, handgun, uniform, vest, backpack, headgear, facewear,
             * binocular, and assigned_items (array of strings). If ret_game_value_ is empty,
             * the loadout is left unchanged.
             *
             * @param ret_game_value_ Serialized loadout array expected to contain at least
             *                        ten elements in the order described above.
             */
            ...
            /**
             * @brief Serializes the unit loadout into a game_value array.
             *
             * @return game_value An array containing, in order: `primary`, `secondary`, `handgun`,
             * `uniform`, `vest`, `backpack`, `headgear`, `facewear`, `binocular`, and the `assigned_items`
             * array.
             */
            explicit operator game_value() const {
            ...
            /**
             * @brief Checks whether this loadout is equal to another loadout.
             *
             * @param other The loadout to compare against.
             * @return true if `primary`, `secondary`, `handgun`, `uniform`, `vest`, `backpack`,
             * `headgear`, `facewear`, `binocular`, and `assigned_items` are all equal to the
             * corresponding fields of `other`, false otherwise.
             */
            ...
            /**
             * @brief Resets the loadout to an empty state.
             *
             * Clears all weapon slots, clothing slots, accessories, binoculars, and assigned items.
             */
            void clear() {
                primary.clear();
                secondary.clear();
