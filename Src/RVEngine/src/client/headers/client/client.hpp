...
namespace intercept {
    namespace client {
        ...
        public:
            ...
            /**
             * @brief Registers a unary SQF command using a legacy uintptr_t-based function pointer.
             *
             * Registers the command under the given name and description and associates it with a legacy
             * function pointer that receives a uintptr_t context and a single `game_value_parameter`.
             *
             * @deprecated Use the new `game_state&`-based `WrapperFunctionUnary` signature instead of uintptr_t.
             *
             * @param name The command name to register (SQF name).
             * @param description Short description of the command.
             * @param function_ Legacy function pointer with signature `game_value(uintptr_t, game_value_parameter)`.
             * @param return_arg_type SQF return type for the registered command.
             * @param right_arg_type SQF right-hand argument type for the registered command.
             * @return registered_sqf_function Handle representing the registered SQF function.
             */
        ...
        public:
            ...
            /**
 * @brief Deleted copy constructor to make invoker_lock non‑copyable.
 */
invoker_lock(const invoker_lock &) = delete;
            /**
 * @brief Disables move construction for invoker_lock.
 *
 * Prevents transferring ownership of the lock; instances cannot be moved.
 */
