...
namespace intercept::client {
    ...
    struct EHIdentifier {
        ...
        /**
         * @brief Determine whether two EHIdentifier instances represent the same event handler identity.
         *
         * @param other The EHIdentifier to compare against.
         * @return `true` if `internal_id`, `arma_eh_id`, and `EHType` are equal; `false` otherwise.
         */
    ...
    struct EHIdentifier_hasher {
        /**
         * @brief Computes a combined hash for an EHIdentifier.
         *
         * @param x The EHIdentifier to hash.
         * @return size_t Hash value produced by combining `internal_id`, `arma_eh_id`, and `EHType`.
         */
    ...
    public:
        /**
 * @brief Constructs an empty EHIdentifierHandle.
 *
 * Creates a handle that does not own or manage any EHIdentifier.
 */
constexpr EHIdentifierHandle() = default;
        /**
 * @brief Construct an EHIdentifierHandle that owns an EHIdentifier and a deletion callback.
 *
 * @param ident The event handler identifier to manage the lifetime of.
 * @param onDelete Callback invoked with the stored `EHIdentifier&` when this handle is destroyed or replaced; intended to perform cleanup/unregistration of the associated event handler.
 */
EHIdentifierHandle(EHIdentifier ident, std::function<void(EHIdentifier&)> onDelete) : handle(std::make_shared<impl>(std::move(ident), std::move(onDelete))) {}
        /**
         * @brief Assigns another EHIdentifierHandle to this handle, transferring ownership semantics and guarding against stale event handler deletions.
         *
         * If this handle and `other` refer to different underlying implementations but share the same event-handler type and Arma EH id while having different internal ids, marks the replaced handle's `already_deleted` flag to prevent a duplicate deletion. Copies the underlying shared implementation pointer from `other`.
         *
         * @param other The source EHIdentifierHandle to assign from.
         * @return EHIdentifierHandle& Reference to this handle after assignment.
         */
    ...
    private:
        ...
        public:
            /**
 * @brief Create an implementation wrapper that holds an event handler identifier and its cleanup callback.
 *
 * @param ident_ The EHIdentifier owned by this implementation.
 * @param onDelete_ Callback invoked with the identifier when the implementation is destroyed or released.
 */
...
#define EH_Add_Mission_definition(name, retVal, fncArg)                                                                                                             \
    ...
    */
    ...
    /**
     * @brief Registers a C++ callback for a specific mission event and returns a lifetime-managed handle.
     *
     * The returned handle owns the registration; destroying the handle unregisters the callback and removes the associated script event handler.
     *
     * @tparam Type The mission event type to bind (one of eventhandlers_mission).
     * @tparam Func Callable invoked when the event occurs; its signature must match the expected arguments for the chosen `Type`.
     * @param fnc The callback to invoke for the event.
     * @return EHIdentifierHandle Handle that manages the registered event handler and removes it on destruction.
     */
...
//#TODO correct type for ammo
 ...
 public:
     /**
          * @brief Construct an eventhandler_hit_part_type from a game_value array.
          *
          * Parses a game_value (expected to be an array) produced by a hit-part event and initializes
          * the struct members from specific array positions.
          *
          * @param gv Game value array where elements map as:
          *  - [0] target
          *  - [1] shooter
          *  - [2] bullet
          *  - [3] position
          *  - [4] velocity
          *  - [5] selection (array -> vector)
          *  - [6] ammo (array -> vector)
          *  - [7] direction
          *  - [8] radius
          *  - [9] surface
          *  - [19] direct
          *
          * Only the listed indices are consumed; other entries are ignored.
          */
...
#define EH_Add_Object_definition(name, retVal, fncArg)                                                                                                            \
    ...
    struct __addEventHandler_Impl<eventhandlers_object::HitPart> {
        ...
        /**
         * @brief Register a HitPart event handler on an object.
         *
         * @param obj Target object to attach the handler to.
         * @param function Callback invoked when the object's HitPart event fires; receives a vector of `eventhandler_hit_part_type` describing the hit parts.
         * @return EHIdentifier Identifier for the registered event handler, which can be used to remove or manage the handler's lifetime.
         */
    ...
    */
    ...
    /**
     * @brief Register a callback for a specific object event.
     *
     * Registers `fnc` as the handler for the compile-time event `Type` on `obj` and returns a handle that manages its lifetime.
     *
     * @tparam Type The object event to bind (one of `eventhandlers_object`).
     * @param obj The object to attach the event handler to.
     * @param fnc The callback invoked when the event fires; required signature depends on `Type`.
     * @return EHIdentifierHandle Handle representing the registered event handler; destroying or releasing the handle unregisters the handler.
     */
...
#define EH_Func_Args_Ctrl_LBDblClick types::control ctrl, int selected_index
    ...
    public:
        ...
        float value_lb{};
        /**
         * @brief Construct an event_handlers_listbox_info from a game_value array.
         *
         * Initializes the struct's text, value, and data fields from the first three
         * elements of the provided `types::game_value`.
         *
         * @param gv A `types::game_value` containing at least three elements: index 0 is used for `text_lb`, index 1 for `value_lb`, and index 2 for `data_lb`.
         */
...
#define EH_Add_Ctrl_definition(name, retVal, fncArg)                                                                                                          \
    ...
    */
    ...
    /**
     * @brief Register a control event handler for the given control and return a lifetime-managing handle.
     *
     * The provided callable is invoked when the specified control event (template parameter `Type`) occurs on `ctrl`.
     *
     * @tparam Type The control event type to bind.
     * @param ctrl The control to attach the handler to.
     * @param fnc Callable invoked when the event fires; it must match the argument signature for the chosen `Type`.
     * @return EHIdentifierHandle Handle that owns the registration. Destroying the handle removes the handler and unregisters the underlying script event.
     */
...
#define EH_Add_MP_definition(name, retVal, fncArg)                                                                                                          \
    ...
    */
    ...
    /**
     * @brief Registers a multiplayer event handler for a specific unit.
     *
     * Associates the provided callable with the given unit for the MP event identified by the template parameter `Type`.
     *
     * @tparam Type The multiplayer event to bind (one of `eventhandlers_mp`).
     * @tparam Func Callable invoked when the event fires; its signature must match the event's expected arguments.
     * @param unit The unit to attach the event handler to.
     * @param fnc The handler to invoke when the event occurs.
     * @return EHIdentifierHandle A handle that owns the registration and will unregister the handler when destroyed.
     */
...
#define EH_Add_Display_definition(name, retVal, fncArg)                                                                                                          \
    ...
    */
    ...
    /**
     * @brief Register a callback for a specific display event and return a lifetime-managed handle.
     *
     * Registers `fnc` as the handler for the display event specified by template parameter `Type` on `disp`.
     *
     * @tparam Type The display event enum value (from `eventhandlers_display`) identifying which event to bind.
     * @tparam Func The callable type of `fnc`.
     * @param disp The display to attach the event handler to.
     * @param fnc Callback invoked when the event occurs; its signature must match the expected argument types for `Type`.
     * @return EHIdentifierHandle A handle that owns the registered display event handler. Destroying or releasing the handle unregisters the handler.
     */
...
#define EH_Add_Group_definition(name, retVal, fncArg)                                                                                                             \
    ...
    */
        ...
        /**
     * @brief Register a C++ callback for a specific group event and obtain a lifetime-managed handle.
     *
     * @param grp The group to attach the event handler to.
     * @param fnc The callback invoked when the event occurs; its parameters depend on the selected event `Type`.
     * @return EHIdentifierHandle Handle that owns the registration. Destroying or replacing the handle unregisters the event handler and removes its internal mapping.
     */
