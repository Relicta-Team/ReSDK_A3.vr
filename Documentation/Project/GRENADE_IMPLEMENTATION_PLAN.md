...
- Treat active-hand switching as irrelevant. The pin is replaced only by the explicit context action or by putting a pin-pulled grenade into the holder's own inventory/container.
...
- Moving a pin-pulled grenade into its current holder's inventory or one of that holder's containers also replaces the pin.
- Giving, dropping, placing, or throwing a pin-pulled grenade releases the lever. Moving an already-burning grenade never stops its fuse.
...
- Add a client concussion event with a sixty-second decay: tinnitus, dynamic blur/flash when the blast is visible, camera shake, and a low-pass filter shared by ordinary positional sounds and remote player voices.
- Hearing effects depend on distance. Flash and blur additionally require the explosion to be on screen with an unobstructed camera line.
...
- Client tests: particle creation without script errors, tinnitus and low-pass decay, voice muffling, sight-gated flash/blur, and repeated explosions.
