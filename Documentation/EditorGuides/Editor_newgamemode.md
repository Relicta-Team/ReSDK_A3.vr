...
Каждый раунд должен в конце концов быть завершен. И на наши плечи ложится ответственность за то, чтобы он заканчивался различными способами.  

### Вывод результата конца раунда
Для вывода результата конца раунда используется метод **getResultTextOnFinish**. Принцип работы аналогичен методу `onFailReasonToStart`, только вызывается он когда режим завершён.
