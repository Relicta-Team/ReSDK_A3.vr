...
4. **Окно сцены.** Основное окно редактирования режима.
...
Ниже представлен список стандартных хоткеев управления сцены:
| Кнопки | Действие |
| --- | --- |
...
| <kbd>Ctrl</kbd> + <kbd>S</kbd> | Сохранить текущую карту в рабочей области |
| <kbd>E</kbd> | Открыть/закрыть окно слоев и игровых объектов на карте |
| <kbd>R</kbd> | Открыть/закрыть окно истории |
| <kbd>Shift</kbd> + <kbd>E</kbd> | Открыть/закрыть окно инспектора |
| <kbd>Shift</kbd> + <kbd>R</kbd> | Открыть/закрыть окно библиотеки игровых объектов 
...
| Интерфейс|Здесь можно включить, выключить или переключить отображение левой или правой панели, а также переключить отображение значков объектов и интерфейса.
|Настройка карты|Здесь можно переключить время суток, отображение курсора геометрии для определения где есть *физическая* поверхность и переключить отображение сетки чанков.
...
|Валидаторы| Здесь можно обновить объекты, которые были перепривязаны от **IStruct** или **Decor**, отобразить в консоли пути всех класов, загруженных в редактор, проверить нагрузку сцены игровыми объектами и найти "*мертвые*" классы, которые есть на карте но нет в проекте.
...
|Редактор позиции модели|Открывает внутренний редактор создания точек относительно модели. Полезный инструмент например для создания кроватей или стульев с помощью точкек на которых можно расположит персонажа. Если бы выделен объект, то он будет помещен в редактор эмиттеров.
|Редактор частиц и освещения|Открывает внутренний редактор системы частиц и освещения. Если бы выделен объект, то он будет помещен в редактор эмиттеров. Подробнее в [справочнике по редактору эмиттеров](Editor_emitters.md)
...
|Настроить симуляцию и запустить|Открывает окно детальной настройки симуляции. Все опции задокументированы при наведении. Для использования требует названия классов режимов и ролей.
...
## Инспектор
Инспектор позволяет редактировать свойства выделенного игрового объекта. В зависимости от его типа (имени класса) список доступных настроек различается.
![Окно инспектора](Data/editor_inspector.png)

| Опция | Описание |
| --- | --- |
|Свойства объекта|Отображает класснейм выбранного объекта
...
> **Обратите внимание!**
>> Операция модификации истории через окно `История изменений` очень затратна для производительности. Чем больше изменений откатывается, тем дольше это занимает время. Во время отката истории исполняемый процесс Arma3 может полностью зависнуть на какое-то время.

### Специфика отката изменений замененной модели
> **Обратите внимание!**
>> При откате изменений, связанных с заменой модели в историю пишется 2 действия: изменение свойств объекта и фактическая замена модели в сцене. Т.е. если бы мы хотели откатить изменения смены модели, то нужно откатиться на 2 строки ниже. Пример:<br/> Мы взяли костёр и изменили модель на другой костер, но нам понадобилось отменить эти изменения. В этом случае нам нужно вернуться к точке раньше изменений свойств перед заменой как на скриншоте ниже.
![Пример правильно отката изменений](Data/editor_history_reset.png)
