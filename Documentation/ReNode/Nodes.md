...
Все объектные узлы вызывают функции или манипулируют свойствами благодаря порту "Цель", который есть у каждого объектного узла (кроме точек входа). Если порт "Цель" отмечен красным цветом, значит в графе не может быть использован этот узел без явного подключения к порту "Цель".
...
Выходные порты, это порты, которые отдают на выход значения, обработанные узлом. Например, пример ниже отдает результат, равный количество клиентов, занявших указанную роль. Так же он посылает импульс на порт "Выход" передавая управление следующему узлу, подключенному к нему.
...
> !!! Внимание: Подробнее про преобразования [тут](#преобразования)
...
4. Преобразование типа. Подробнее [ниже](#преобразования)
...
На автоматические порты действую ограничения на подключения, которые можно посмотреть при выведении описания узла.

<img src="Data/nodes_auto_desc.png" width="500px">

На изображении выше показано описания узла сложения двух чисел. Из описания становится ясно, что автоматические порты этого узла принимают значения дробных и целых чисел. Соответственно, например массивы строк или объекты не могут быть подключены в этот автоматический порт.
...
## Различия узлов
В примерах выше вы могли разглядеть некоторую разницу между узлами: какие-то из них не имеют портов выполнения (белые треугольники). В этом разделе мы разберем какие бывают типы узлов.
...
### Чистые узлы
Чистыми являются узлы, выполняющие простые операции, получающие значения объектов без сложных расчетов, либо вычисляющих какие-то значения на основе входных данных без изменения входных данных.
...
Он предназначен для установки совместимости с дверями, которые можно открыть. При выполнении этого узла в объекте ключа, подключенном к порту "Цель" будет изменено значение с информацией о типах замков, которые можно открыть ключом.


# Основные операторы

Основные операторы находятся в библиотеке узлов раздела "Операторы" 
