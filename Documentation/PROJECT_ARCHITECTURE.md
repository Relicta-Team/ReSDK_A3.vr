...
> Названия, выделенные *курсивом* текстом являются папками с API нижнего уровня. На них строится основа клиента или сервера и к редактированию содержимого этих модулей нужно подходить с должным вниманием.

## Базовые директории
В папке `src` есть несколько папок:

* client - Здесь всё, что связано с работой на клиенте. Весь код из этой папки есть у конечного клиента при подключении к серверу.
...
* *ScriptErrorHandler* - Обработчик ошибок.
...
- `host\oop.hpp` - заголовочный файл для классов и работы с объектами. Там где есть классы или управление объектами, созданными из них используется этот заголовочный файл (пример: Игровые объекты, режимы, роли и т.д.). Этот заголовочный файл используется только в серверных модулях.
...
Создание классов реализовано с помощью групп макросов:
```sqf
